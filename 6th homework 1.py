...
findmax.calculate()
print '     max range with air drag     '          # give max range with air resistance
...
findmax= FIND_MAX(x,v,theta)
findmax.calculate()
print '     max range without air drag    '    # give max range WITHOUT air resistance
print x[findmax.xmax_posi],'m     ',v[findmax.xmax_posi],'m/s    ',theta[findmax.xmax_posi],'degree'
pl.xlabel('x(m)',fontsize=18)
pl.xlim(0,60000)
pl.ylim(0,26000)
pl.annotate(r'$v_{0}$'+'= '+'700.00 m/s',
            xy=(8000,24000), fontsize=15
            )            
pl.annotate('with and without air drag',
            xy=(15000,22000), fontsize=15
            )
pl.show(figure)
